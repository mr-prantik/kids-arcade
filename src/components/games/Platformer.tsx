...
// Proper callback parameter type for Phaser overlap/collision
type ArcadePhysicsCallbackParams =
...
class RunnerScene extends Phaser.Scene {
  ...
  create() {
    ...
    const staticGround = this.physics.add.staticGroup();
    staticGround
      .create(this.scale.width / 2, this.scale.height - 32, "ground")
      .setScale(this.scale.width / 200, 1)
      .refreshBody();
  ...
  spawnTree() {
    ...
    // Fix readonly body.allowGravity typing issue
    if (tree.body) {
      (tree.body as Phaser.Physics.Arcade.Body).allowGravity = false;
    }

    // hitbox only trunk
    tree.setSize(30, 60).setOffset(35, 40);

    // sometimes add a platform above
    if (Phaser.Math.Between(0, 100) < 40) this.spawnPlatform();
  }
  ...
  // === COLLISION HANDLERS ===
  collectCoin = (
    _player: ArcadePhysicsCallbackParams,
    coinObj: ArcadePhysicsCallbackParams
  ) => {
    const coin = coinObj as Phaser.Physics.Arcade.Sprite;
    coin.disableBody(true, true);
    this.score += 50;
  ...
  hitObstacle = (
    playerObj: ArcadePhysicsCallbackParams,
    _obs: ArcadePhysicsCallbackParams
  ) => {
    const player = playerObj as Phaser.Physics.Arcade.Sprite;
    if (this.gameOver) return;
    this.gameOver = true;
  ...
  update(time: number, delta: number) {
    ...
    this.trees.getChildren().forEach((c) => {
      if ((c as Phaser.Physics.Arcade.Sprite).x < -100) c.destroy();
    });
    this.birds.getChildren().forEach((c) => {
      if ((c as Phaser.Physics.Arcade.Sprite).x < -100) c.destroy();
    });
    ...
    this.platforms.getChildren().forEach((c) => {
      if ((c as Phaser.Physics.Arcade.Sprite).x < -150) c.destroy();
    });
  }
}
